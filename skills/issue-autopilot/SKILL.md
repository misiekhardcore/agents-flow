...
- Stages 1–5: Execute per [Ref: references/stages.md], pausing at human decision points (after define, after impl, after review, before merge, complete).

**State machine**: Resume state machine in [Ref: references/stages.md] determines which stage to enter on each invocation based on issue/PR/branch state.

## Process Flow

1. Run `${CLAUDE_PLUGIN_ROOT}/_shared/repo-preflight.md` and pause for confirmation.
2. Detect state using commands in [Ref: references/stages.md] § State detection.
