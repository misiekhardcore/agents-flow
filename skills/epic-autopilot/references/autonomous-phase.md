...
2. Dispatch all sub-tasks in current tier as parallel `Task` sub-agents **in a single message**. For each sub-issue M:
   ...
   - Create worktree for branch `feat/epic-<N>-sub-<M>` on base `<base-branch>`. See `Invoke Skill("worktree")`.
   - Pass seed brief to sub-agent's `/implement` invocation. See `Invoke Skill("specialist-mode")` with overrides:
     - `scope_class`: `Deep`
