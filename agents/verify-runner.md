...
2. **Evaluate migration gate**: if gate fires, plan to spawn `Agent("agents/reviewer-migration.md")` alongside qa-agents.
3. **Spawn in parallel**:
   - One `Agent("agents/qa-agent.md")` per AC group. Pass `ac_group` and `diff`.
